...
describe('Overview selectors', () => {
    ...
    describe('getStartDate', () => {
        it('should get the start date', () => {
            expect(S.getStartDate(state)).to.deep.equal(DateTime.fromISO('2018-01-31T23:59:59.999Z'));
        });
    });
    describe('getEndDate', () => {
        it('should get the end date', () => {
            expect(S.getEndDate(state)).to.deep.equal(DateTime.fromISO('2018-06-30T22:59:59.999Z'));
        });
    });
    describe('getNumRows', () => {
        it('should get the numRows', () => {
            expect(S.getNumRows(state)).to.equal(7);
        });
    });

    describe('getBalance', () => {
        it('should get the balance items', () => {
            expect(S.getBalance(state).toJS()).to.deep.equal([13502, 19220, 11876, 14981, 14230, 12678, 0]);
        });
    });
    ...
    describe('getCurrentDate', () => {
        ...
        it('should get the end of the current day', () => {
            result = S.getCurrentDate(state);

            expect(result).to.deep.equal(DateTime.fromISO('2018-03-23T23:59:59.999Z'));
        });
        it('should not reload the result if the day doesn\'t change', () => {
            const nextResult = S.getCurrentDate(state.set('now', DateTime.fromISO('2018-03-23T15:20Z')));
    ...
    describe('getFutureMonths', () => {
        it('should calculate the number of months in the future there are, based on the current date', () => {
            expect(S.getFutureMonths(state)).to.equal(3);
            expect(S.getFutureMonths(state.set('now', DateTime.fromISO('2018-03-31T15:20Z'))))
                .to.equal(3);
            expect(S.getFutureMonths(state.set('now', DateTime.fromISO('2018-03-31T22:59Z'))))
                .to.equal(3);

            expect(S.getFutureMonths(state.set('now', DateTime.fromISO('2018-03-31T23:00Z'))))
                .to.equal(2);
        });
    });

    describe('getRowDates', () => {
        it('should get a list of dates at the end of each month', () => {
            expect(S.getRowDates(state).toJS()).to.deep.equal([
    ...
    describe('getProcessedCost', () => {
        it('should process the cost data, including making predictions, adding spending / net columns etc.', () => {
            expect(S.getProcessedCost(state).toJS()).to.deep.equal({
    ...
    describe('getOverviewTable', () => {
        it('should get a list of rows for the overview table', () => {
            expect(S.getOverviewTable(state).toJS()).to.deep.equal([
                {
                    past: true,
                    active: false,
                    future: false,
