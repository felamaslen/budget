...
test('handling date input', t => {
    transactions.forEach((transaction, index) => {
        ...
        const {
            childNodes: [inputDate],
        } = dateInputCol.childNodes[0];
...
test('handling units input', t => {
    transactions.forEach((transaction, index) => {
        ...
        const {
            childNodes: [inputUnits],
        } = unitsInputCol.childNodes[0];
...
test('adding a transaction', t => {
    ...
    const {
        childNodes: [fieldDate],
    } = rowDate.childNodes[1];
    const {
        childNodes: [fieldUnits],
    } = rowUnits.childNodes[1];
    const {
        childNodes: [fieldCost],
    } = rowCost.childNodes[1];
