...
import {
    DATA_KEY_ABBR, CREATE, UPDATE, DELETE,
...
test('Actions intended for other pages are ignored', (t) => {
    ...
    t.deepEqual(myListReducer(initialStateDelete, listItemDeleted('some-id', {
        page: 'other-page',
    })), initialStateDelete);
});
...
test('SYNC_RECEIVED updates optimistically-created items with their real IDs', (t) => {
    ...
    t.deepEqual(result.items[0], {
        id: 'real-id-b', some: 'prop', is: true, __optimistic: null,
    });
});
...
test('SYNC_RECEIVED marks optimistically-updated items as confirmed', (t) => {
    ...
    t.is(result.items.length, 1);
    t.deepEqual(result.items[0], {
        id: 'real-id-z', some: 'prop', is: true, __optimistic: null,
    });
