...
function getColorFromScore(color, score, negative) {
  if (!color) {
    errorMessages.newMessage("No colour given to getColor!", 0, C.MSG_TIME_DEBUG);
    color = [36, 191, 55];
  }
...
function validateDateInput(val) {
  ...
  if (!isDate) {
    errorMessages.newMessage("\"" + val + "\" isn\"t a date", 0, C.MSG_TIME_DEBUG);
...
function validateCurrencyInput(val) {
  ...
  if (isNaN(floatVal) || val.match(/[A-Za-z]/)) {
    errorMessages.newMessage("\"" + val.toString() + "\" isn\"t a number", 0, C.MSG_TIME_DEBUG);
    ...
    $.ajax({
      ...
      success: (data) => {
        if (data && data.error === false) {
          ...
          errorMessages.newMessage(data.errorText, 1, C.MSG_TIME_WARN);
          if (error) {
      ...
      error: () => {
        errorMessages.newMessage("General API error!", 2, C.MSG_TIME_ERROR);
        if (error) {
          error();
        }
      },
...
class User {
  ...
  onLoginSuccess(data) {
    ...
    if (typeof Storage !== "undefined") {
      ...
      errorMessages.newMessage(C.E_NO_STORAGE, 1, C.MSG_TIME_WARN);
    }

    $("#nav-link-" + currentPage).trigger(C.NAV_HANDLE_EVENT);
...
class InlineEdit {
  ...
  finish(callback) {
    ...
    if (this.locked) {
      ...
      errorMessages.newMessage("Tried to finish editing while locked", 0, C.MSG_TIME_DEBUG);
      return false;
    }
    ...
    errorMessages.newMessage("Tried to finish editing while no hook set", 0, C.MSG_TIME_DEBUG);
...
class Graph {
  constructor(options) {
    ...
    if (!this.supported) {
      errorMessages.newMessage("HTML5 Canvas is not supported! Not drawing graphs", 3, C.MSG_TIME_FATAL);
    }
...
class LineGraph extends Graph {
  ...
  setLogY() {
    if (this.minY * this.maxY <= 0) {
      ...
      errorMessages.newMessage("Attempted to set log range containing zero!", 0, C.MSG_TIME_DEBUG);
      return;
    }
...
class GraphBalance extends LineGraph {
  constructor(options) {
    ...
    this.colors = [C.COLOR_BALANCE_ACTUAL, C.COLOR_BALANCE_PREDICTED];
    this.stroke = true;
  ...
  drawKey() {
    ...
    this.ctx.font = C.FONT_GRAPH_TITLE;
    this.ctx.fillStyle = C.COLOR_GRAPH_TITLE;
    ...
    this.ctx.strokeStyle = C.COLOR_BALANCE_ACTUAL;
    this.ctx.moveTo(50, 40);
    this.ctx.lineTo(74, 40);
    this.ctx.stroke();
    this.ctx.closePath();

    this.ctx.font = C.FONT_GRAPH_KEY_SMALL;
    this.ctx.textBaseline = "middle";
    this.ctx.fillStyle = C.COLOR_DARK;
    this.ctx.fillText("Actual", 78, 40);

    this.ctx.beginPath();
    this.ctx.lineWidth = 2;
    this.ctx.strokeStyle = C.COLOR_BALANCE_PREDICTED;
  ...
  draw() {
    ...
    // draw axes
    this.ctx.strokeStyle = C.COLOR_LIGHT_GREY;
    this.ctx.lineWidth = 1;

    this.ctx.font = C.FONT_AXIS_LABEL;
    this.ctx.fillStyle = C.COLOR_DARK;
    this.ctx.textBaseline = "top";
    ...
    const numTicks = C.GRAPH_BALANCE_NUM_TICKS * minorTicks;
    ...
    for (let i = 0; i < numTicks; i++) {
      ...
      this.ctx.strokeStyle = major ? C.COLOR_LIGHT_GREY : C.COLOR_LIGHT;
      this.ctx.stroke();
      this.ctx.closePath();
    }
    ...
    this.dataAnomalies.forEach((anomaly, key) => {
      if (Math.abs(anomaly) > 0) {
        ...
        const color = above ? C.COLOR_PROFIT : C.COLOR_LOSS;
...
class GraphSpend extends LineGraph {
  ...
  draw() {
    ...
    this.ctx.textBaseline = "middle";
    this.ctx.font = C.FONT_GRAPH_KEY;

    const fontColor = C.COLOR_DARK;
    ...
    this.ctx.fillRect(
      C.GRAPH_KEY_OFFSET_X, C.GRAPH_KEY_OFFSET_Y, C.GRAPH_KEY_SIZE, C.GRAPH_KEY_SIZE
    );

    this.ctx.fillStyle = rgb(this.textColors.food);
    this.ctx.fillRect(57, C.GRAPH_KEY_OFFSET_Y, C.GRAPH_KEY_SIZE, C.GRAPH_KEY_SIZE);

    this.ctx.fillStyle = rgb(this.textColors.general);
    this.ctx.fillRect(115, C.GRAPH_KEY_OFFSET_Y, C.GRAPH_KEY_SIZE, C.GRAPH_KEY_SIZE);

    this.ctx.fillStyle = rgb(this.textColors.holiday);
    this.ctx.fillRect(185, C.GRAPH_KEY_OFFSET_Y, C.GRAPH_KEY_SIZE, C.GRAPH_KEY_SIZE);

    this.ctx.fillStyle = rgb(this.textColors.social);
    this.ctx.fillRect(250, C.GRAPH_KEY_OFFSET_Y, C.GRAPH_KEY_SIZE, C.GRAPH_KEY_SIZE);
  }
...
class GraphFundItem extends LineGraph {
  constructor(options) {
    ...
    this.colors = [C.COLOR_GRAPH_FUND_ITEM];
    this.lineWidth = C.GRAPH_FUND_ITEM_LINE_WIDTH;
    this.tension = C.GRAPH_FUND_ITEM_TENSION;
  ...
  draw() {
    ...
    if (this.popout || (this.maxY - this.minY) < this.height / 2) {
      ...
      for (let i = Math.floor(this.minY / inc) * inc; i <= this.maxY; i += inc) {
        ...
        this.ctx.lineTo(this.width - this.padX2, tickPos);
        this.ctx.closePath();
        this.ctx.strokeStyle = i < 0 ? C.COLOR_LOSS_LIGHT : C.COLOR_PROFIT_LIGHT;
        this.ctx.stroke();
      }
    }
...
class GraphFundHistory extends LineGraph {
  constructor(options) {
    ...
    this.tension    = C.GRAPH_FUND_HISTORY_TENSION;
    ...
    this.colorMajor = C.COLOR_GRAPH_FUND_LINE;
    ...
    windowSize.narrow(() => {
      this.resize(C.GRAPH_FUND_HISTORY_WIDTH_NARROW);
    })
    .wide(() => {
      this.resize(C.GRAPH_FUND_HISTORY_WIDTH);
    })
    .trigger();
  }
  ...
  buildStockViewer() {
    this.stocksRefreshInterval = C.STOCKS_REFRESH_INTERVAL;
    this.hlTime = C.STOCKS_REFRESH_INTERVAL - 1000;
  ...
  loadStocksList() {
    if (!C.DO_STOCKS_LIST || this.stocksListLoading) {
      return;
    }
  ...
  onStocksListError() {
    errorMessages.newMessage("Error loading stocks list!", 2, C.MSG_TIME_ERROR);
  }
  ...
  onStockPricesFail() {
    errorMessages.newMessage("Error loading stock prices!", 2, C.MSG_TIME_ERROR);
  }
  ...
  calculateZoomedRange() {
    ...
    this.tickSizeY = getTickSize(
      minY, maxY, C.GRAPH_FUND_HISTORY_NUM_TICKS
    );
  ...
  draw() {
    ...
    const axisColor = C.COLOR_DARK;
    const axisTextColor = C.COLOR_LIGHT;

    const stocksWidth = C.DO_STOCKS_LIST ? C.STOCKS_LIST_WIDTH : 0;
    ...
    this.ctx.font = C.FONT_AXIS_LABEL;
    this.ctx.fillStyle = C.COLOR_LIGHT;
    this.ctx.textAlign = "left";
    this.ctx.textBaseline = "bottom";
    ...
    timeTicks.forEach(tick => {
      ...
      this.ctx.beginPath();
      this.ctx.strokeStyle = tick.major ? C.COLOR_LIGHT : C.COLOR_LIGHT_GREY;
    ...
    // plot past data
    if (this.data) {
      this.lineWidth = C.GRAPH_FUND_HISTORY_LINE_WIDTH;
    ...
    if (
      ...
      const left = align > 0
        ? "initial" : hlX + C.GRAPH_FUND_HISTORY_POINT_RADIUS;
      const right = align > 0
        ? this.width - hlX + C.GRAPH_FUND_HISTORY_POINT_RADIUS : "initial";
      const top = hlY + C.GRAPH_FUND_HISTORY_POINT_RADIUS;
      ...
      this.ctx.arc(
        hlX, hlY, C.GRAPH_FUND_HISTORY_POINT_RADIUS, 0, Math.PI * 2, false
      );

      this.ctx.fillStyle = C.COLOR_GRAPH_FUND_POINT;
      this.ctx.fill();
      this.ctx.closePath();
    }
    else {
...
class PieGraph extends Graph {
  ...
  labelRadiusExtension(x) {
    return x < C.PIE_LABEL_SWITCH_POINT
    ? C.PIE_LABEL_SCALE_FACTOR_PRE * Math.sin(
      C.pio2 * x / C.PIE_LABEL_SWITCH_POINT
    )
    : -C.PIE_LABEL_SCALE_FACTOR_POST * (x - 1) /
      (1 - C.PIE_LABEL_SWITCH_POINT);
  }
  ...
  drawLabel(p, x, y, angle, thisAngle, radius, smallLabelOffset, lastLabelAngle) {
    ...
    if (
      ...
      const quadrant = Math.floor((midAngle + C.pio2) / C.pio2) % 4;

      let labelRadiusScale = C.PIE_LABEL_RADIUS_START;

      if (quadrant === 3) {
        // fraction of the top-left quadrant where the label is
        const frac = (midAngle - Math.PI) / C.pio2;

        if (frac >= C.PIE_LABEL_SWITCH_POINT) {
          labelDirection = 1;
        }

        labelRadiusScale = C.PIE_LABEL_RADIUS_START + C.PIE_LABEL_RADIUS_SCALE *
      ...
      const labelBegin = this.pointFromCircle(
        x, y, radius * C.PIE_LABEL_INSIDE_RADIUS, midAngle
      );
  ...
  draw() {
    ...
    this.ctx.strokeStyle = C.COLOR_GRAPH_TITLE;
    this.ctx.font = C.FONT_AXIS_LABEL;

    let smallLabelOffset = C.PIE_SMALL_LABEL_OFFSET;
    ...
    this.ctx.lineTo(centreX + radius, centreY + C.PIE_DEPTH);
    this.ctx.arc(centreX, centreY + C.PIE_DEPTH, radius, 0, Math.PI, false);
    ...
    this.ctx.font = C.FONT_GRAPH_TITLE_LARGE;
    this.ctx.textAlign = "right";
    this.ctx.textBaseline = "top";
...
class PageList extends Page {
  ...
  addNew() {
    ...
    for (const j of this.colEdit) {
      ...
      if (col === "item" && val.length === 0) {
        errorMessages.newMessage("Must enter text for main item field", 1, C.MSG_TIME_WARN);
        error = true;
      }

      if (val === null) {
        errorMessages.newMessage("Must enter valid data", 1, C.MSG_TIME_WARN);
        error = true;
      }
  ...
  onSubmitError() {
    errorMessages.newMessage("Error updating value! (Server error)", 2, C.MSG_TIME_ERROR);
  }
  ...
  triggerListEdit(column, type, callback) {
    ...
    if (!status) {
      // invalid data input
      errorMessages.newMessage("invalid data input", 1, C.MSG_TIME_WARN);
  ...
  hookDataLoadedAfterRender(callback, res) {
    ...
    if (this.history.history.length > 0) {
      ...
      this.graphFundHistory = new GraphFundHistory({
        width:  C.GRAPH_FUND_HISTORY_WIDTH,
        height: this.pieHeight,
        $cont:  this.$graphs,
        $list:  this.$cont,
        page:   this.page,
...
class PageAnalysis extends Page {
  constructor() {
    ...
    this.treeWidth  = C.ANALYSIS_VIEW_WIDTH;
    this.treeHeight = C.ANALYSIS_VIEW_HEIGHT;
  }
...
const pageDefFunds = {
  ...
  drawPie: true,
  pieStretch: 1.2,
  pieWidth: C.GRAPH_FUNDS_PIE_WIDTH,
  pieLabelLength: 13
};
...
class AutoSearch {
  constructor($input, page, col) {
    ...
    this.timer = null;
    this.throttleTime = C.SEARCH_SUGGESTION_THROTTLE_TIME;
  ...
  suggestionsError() {
    errorMessages.newMessage("Error loading suggestions!", 2, C.MSG_TIME_ERROR);
  }
...
$(document).ready(() => {
  ...
  $("#nav-link-logout").on(C.NAV_HANDLE_EVENT, () => user.logout());
