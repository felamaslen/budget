...
function getSumByCategory(categories, subcategories, entries, categoryName) {
    ...
    const valuesFiltered = values.filter(({ subcategory }) => subcategories.some(
...
const sumByType = (categoryType, categories, subcategories, { currencies, values }) => sumValues(
    currencies,
    values.filter(({ subcategory }) => subcategories.some(({ id, categoryId }) => (
        id === subcategory && categories.some(({ id: compare, type }) => (
            compare === categoryId && type === categoryType
        ))
    ))),
);
...
const withTableProps = (rows) => rows.map(({
    id, date, assets, liabilities, expenses, fti,
}) => ({
    id,
    date,
    assets,
...
const subcategoryPending = (categories) => (categoryId) => categories.some(
    ({ id, __optimistic }) => id === categoryId && __optimistic === CREATE,
);
...
const groupPending = (categories, subcategories) => ({ subcategory }) => subcategories.some(
    ({ id, categoryId, __optimistic }) => id === subcategory
