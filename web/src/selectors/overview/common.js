...
export const getSpendingColumn = (dates) => (data) => ({
    ...data,
    spending: dates.map((date, index) => spendingCategories.reduce((sum, category) => sum + data[category][index], 0)),
});
...
export const getFutureMonths = createSelector(
    getCurrentDate,
    getEndDate,
    (currentDate, endDate) => getMonthDiff(currentDate, endDate),
);
