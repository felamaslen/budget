...
export const getAllPageRows = createSelector(
    getNonFilteredItems,
    withoutDeleted,
);
...
function makeMemoisedRowProcessor() {
    ...
    return (page, now, items) => {
        ...
        const getDaily = PAGES[page].daily ? makeGetDaily(sortedByDate) : () => ({});

        const [result] = sortedByDate.reduce(
            ([last, wasFuture, lastDailySum], { __optimistic, ...item }, index) => {
                ...
                const processedItem = { ...item, future, firstPresent };
                if (PAGES[page].daily) {
                    processedItem.daily = daily;
                }
                ...
                if (
                    ...
                    Object.keys(processedItem).every(key => processedItem[key] === cachedItem[key])
                ) {
...
const getAllNonFilteredItems = state =>
    PAGES_LIST.map(page => ({
        page,
...
const withTransmitValues = requests =>
...
const withCreateRequests = (page, rows) => last =>
    last.concat(
        rows
...
const withUpdateRequests = (page, rows) => last =>
    last.concat(
        rows
...
const withDeleteRequests = (page, rows) => last =>
    last.concat(
        rows
...
const getCrudRequestsByPage = (page, items) =>
    compose(
...
export const getCrudRequests = createSelector(
    getAllNonFilteredItems,
    itemsByPage =>
        itemsByPage.reduce(
            (last, { page, items }) => last.concat(getCrudRequestsByPage(page, items)),
            [],
        ),
);
