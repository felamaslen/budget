...
const getTimeOffsets = createSelector(
    getPrices,
    prices =>
            ...
            .map(({ id, transactions, ...rest }) => ({
                ...
                transactionsToDate: prices[id].values.map((price, index) =>
                    transactions.filter(
                        ({ date }) =>
                            Number(date) <
                            1000 * (startTime + cacheTimes[index + prices[id].startIndex]),
                    ),
                ),
            })),
);
...
const getSoldById = createSelector(
    [getViewSoldFunds, getItemsWithInfo],
    (viewSold, items) =>
        items.reduce(
    ...
    (
        ...
        const getFundLinesByMode = (mode: Mode): FundLine[] =>
            fundItems.reduce((last: FundLine[], { id, color }): FundLine[] => {
                ...
                const lines: Data[] = getFundLineProcessed(
                    times[id],
                    timeOffsets,
                    { prices, units, costs },
                    mode,
