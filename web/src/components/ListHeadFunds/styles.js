...
import { FlexCenter } from '~client/styled/shared/layout';
...
const getGainColor = isMobile => ({ profit, loss }) => {
    if (isMobile) {
        if (profit) {
...
const getGainColorLight = isMobile => ({ profit, loss }) => {
    return compose(
        desaturate(0.6),
        lighten(0.1),
    )(getGainColor(isMobile)({ profit, loss }));
};

export const OverallGain = styled(FlexCenter)`
    width: 100%;
    height: ${rem(42)};
    text-transform: none;
