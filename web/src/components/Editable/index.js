...
function EditableField({ id, item, onChange, ...rest }) {
  ...
  if (item === 'date') {
    return <FormFieldDate inline label={`date-input-${id}`} {...props} />;
  }
  if (item === 'cost') {
    return <FormFieldCost inline label={`cost-input-${id}`} {...props} />;
  }
  if (item === 'transactions') {
    return <FormFieldTransactions create {...props} />;
  }

  return <FormFieldText inline label={`${item}-input-${id}`} {...props} />;
}
