...
import { STEP_VALUES, STEP_DATE } from './constants';
...
export const EditByCategoryValue = styled.div`
    ...
    background: linear-gradient(
        to bottom,
        ${colors['translucent-l2']},
        ${colors['translucent-l1']}
    );
`;
...
export const SectionSubtitle = styled.h6`
    ...
    ${EditByCategoryGroup} & {
        ...
        &::before {
            ...
            height: 0;
            right: 15px;
            top: ${({ hidden }) => (hidden ? 10 : 5)}px;
            padding: 2px;
            border-width: 0 3px 3px 0;
            border: solid black;
            border-left-color: transparent;
            border-top-color: transparent;
            transform: rotate(${({ hidden }) => (hidden ? 225 : 45)}deg);
        }
    }
`;
