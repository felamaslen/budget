...
import { colors, sizes } from '../../styled/variables';
import { rem } from '../../styled/mixins';

const spin = (offset = 15) => keyframes`
    from {
...
export const Progress = styled.div`
    ...
    text-indent: -1000px;
    overflow: hidden;
    animation: ${({ offset }) => spin(offset)} 1s infinite steps(8);
    ...
    &::before {
        background: ${({ offset }) => darken(0.8 - (offset / 360) * 1.5, colors.light)};
    }
    &::after {
        transform: rotate(-45deg);
        background: ${({ offset }) => darken(0.8 - ((offset + 45) / 360) * 1.5, colors.light)};
    }
`;
