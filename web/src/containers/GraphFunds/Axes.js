...
function calculateTicksY({ tickSizeY, minY, maxY, pixY }) {
    ...
    const numTicks =
        typeof tickSizeY === 'undefined' || Number.isNaN(tickSizeY)
            ? 0
    ...
    return new Array(numTicks).fill(0).map((item, key) => {
...
export default function Axes({ startTime, tickSizeY, mode, minY, maxY, minX, maxX, pixX, pixY }) {
    ...
    const ticksY = calculateTicksY({
        ...
        <g key={value}>
            <line x1={x0} y1={pos} x2={xMax} y2={pos} stroke={axisColor(value)} strokeWidth={1} />
            <text
                x={xMax}
                y={pos - 2}
                color={textColor}
    ...
    const timeTicksSmall = timeTicks.map(({ pix, major }, key) => (
        <line
            ...
            y1={y0}
            x2={pix}
            y2={y0 - getTickSize(major)}
            stroke={getColor(major)}
            strokeWidth={1}
        />
    ));

    const timeTicksMajor = timeTicks
        .filter(({ major }) => major > 1)
        .map(({ pix, major }, key) => (
            <line
                key={key}
                x1={pix}
                y1={y0 - getTickSize(major)}
                x2={pix}
                y2={yMax}
                stroke={axisColor(1)}
                strokeWidth={1}
    ...
    const timeTicksText = timeTicks
        .filter(({ text }) => text)
        .map(({ text, pix, major }) => (
            <text
                key={pix}
                x={pix}
                y={y0 - getTickSize(major)}
                fontFamily={fontFamily}
                fontSize={fontSize}
                alignmentBaseline="baseline"
