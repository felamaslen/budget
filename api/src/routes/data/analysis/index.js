...
function getRowsByDate(results) {
    return results.reduce((items, rows, categoryKey) => rows.reduce((itemsByDate, { date, cost }) => {
...
async function getPeriodCost(db, user, now, params) {
    ...
    const totalCost = costs.reduce((sum, result) => result.reduce((resultSum, { cost }) => (
        resultSum + Number(cost)
    ), sum), 0);
    ...
    return {
        timeline, cost: itemCost, saved, description,
    };
}
