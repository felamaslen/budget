...
import {
    getBroker,
    getEligibleFunds,
} from '~api/scripts/scrape-funds/process';
...
test('getBroker returning  HL for valid fund names', (t) => {
    t.is(getBroker(config, TEST_FUND_NAMES[0]), 'hl');
    t.is(getBroker(config, TEST_FUND_NAMES[1]), 'hl');
});

test('getBroker throwing an error for invalid fund names', (t) => {
    t.throws(() => getBroker('foo'));
});
...
test('getEligibleFunds maps and filters funds by validity', (t) => {
    ...
    t.deepEqual(getEligibleFunds(config, logger, queryResultAllInvalid), []);
    ...
    t.deepEqual(getEligibleFunds(config, logger, queryResultSomeInvalid), [
        {
            uid: 1,
...
test('getEligibleFunds filters funds by uniqueness', (t) => {
    ...
    t.deepEqual(getEligibleFunds(config, logger, queryResultSomeDuplicate), [
        {
            uid: 1,
            hash: fundHash(TEST_FUND_NAMES[1], config.data.funds.salt),
            broker: 'hl',
