...
describe('Planning utils (calculations)', () => {
  ...
  describe(calculateMonthlyIncomeTax.name, () => {
    ...
    it.each`
      taxpayer                    | taxableIncome | taxCode    | pensionContrib | expected
    ...
    `(
      'should calculate pension tax relief for a $taxpayer tax payer',
      ({ taxableIncome, taxCode, pensionContrib, expected }) => {
        expect.assertions(1);
        expect(
          calculateMonthlyIncomeTax(
            taxableIncome,
