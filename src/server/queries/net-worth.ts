...
export const getCategories = withDb<Category | readonly Category[]>(
  async (
...
export const getSubcategories = withDb<Subcategory | readonly Subcategory[]>(
  async (
    db: DatabasePoolConnectionType,
    socket: SocketWithAuth,
    data: SpecificItem | undefined,
...
export const getEntries = withDb<Entry | readonly Entry[]>(
  async (
    db: DatabasePoolConnectionType,
    socket: SocketWithAuth,
    data: SpecificItem | undefined,
    ...
    where ${sql.join(
      [
        sql`uid = ${socket.handshake.user.uid}`,
        whereId(data),
